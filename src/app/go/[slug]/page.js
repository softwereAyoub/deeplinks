...
//       // 1. جلب البيانات والتحقق من الصلاحية
//       const { data: linkData, error: linkError } = await supabase
//         .from('links')
//         .select(`
//           id, 
...
//       // التحقق من الاشتراك وانتهاء الصلاحية
//       const ownerIsSubscribed = linkData.profiles?.is_subscribed || false;
//       const expirationDate = linkData.expires_at ? new Date(linkData.expires_at) : null;
//       const now = new Date();
...
//       // 2. تحديث الإحصائيات (Increment Clicks)
//             const today = new Date().toISOString().split('T')[0];
//       try {
//         await supabase
//           .from('links')
...
//       // 4. بناء الرابط العميق (Deep Link Logic)
//       if (platform === 'youtube') {
//         const videoId = originalUrl.split('v=')[1]?.split('&')[0] || originalUrl.split('/').pop();
//         if (isAndroid) {
//           // Intent مع Fallback خاص لتيك توك
...
//       // 5. تنفيذ التحويل (Execution)
//       // انتظار بسيط لظهور التصميم
//       await new Promise(resolve => setTimeout(resolve, isTikTok ? 300 : 800));

//       if (isTikTok) {
//         // سيناريو تيك توك: إظهار الزر فوراً + محاولة فتح خجولة
//         setShowManualButton(true);
//         window.location.href = deepLink;
//       } else {
//         // سيناريو إنستغرام/فيسبوك: محاولة فتح قوية + زر احتياطي
//         window.location.href = deepLink;
//         setTimeout(() => {
//           setShowManualButton(true);
//         }, 1500);
...
//   // دالة التعامل مع الزر اليدوي (The Smart Button)
//   const handleManualClick = () => {
//     setLoading(true);
...
//     // إذا لم يخرج المستخدم من الصفحة خلال 2 ثانية، نوجهه للمتصفح
//     const failTimeout = setTimeout(() => {
//       setLoading(false);
//       window.location.replace(original_urle);
//     }, 2000);

//     // إذا غادر الصفحة (نجح التطبيق)، نلغي التحويل للمتصفح
//     window.onblur = () => {
//       setLoading(false);
//       clearTimeout(failTimeout);
//     };
...
//           <h1 className="text-2xl font-black text-slate-800 mb-2">Link Expired</h1>
//           <p className="text-slate-500 text-sm">This link is no longer available.</p>
//         </div>
//       </div>
//     );
//   }
